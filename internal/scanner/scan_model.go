...
// BannerIdentify 根据端口和Banner关键词识别服务
func BannerIdentify(port int, bann string) string {
	// 第一层：Banner 关键词匹配（最准）
	if bann != "" {
		b := strings.ToLower(bann)
...
func ScanFullPort(ip string, host string, grabbanner bool) []Portresult {
...
func ScanFastPorts(ip string, host string, grabbanner bool) []Portresult {
	topPorts := TopPort()
	var openports []Portresult
	var mu sync.Mutex
	var wg sync.WaitGroup

	for port := range topPorts {
		wg.Add(1)
		go func(p int) {
			defer wg.Done()
			if IsportAlive(ip, p) {
				...
				if grabbanner {
					if p == 80 || p == 443 || p == 8080 || p == 3128 || p == 8081 || p == 9090 {
						info, err = banner.Httpbannerget(ip, p, host)
						if err != nil {
							fmt.Println(err)
							bann = ""
						} else {
				...
				openports = append(openports, Portresult{Port: p, Banner: bann, Server: BannerIdentify(p, bann), Fingerprints: fps})
				mu.Unlock()
			}
		}(port)
	}
...
func ScanTopPorts(ip string, host string, grabbanner bool) []Portresult {
	top100ports := top1000ports()
	var openports []Portresult
	var mu sync.Mutex
	var wg sync.WaitGroup

	for _, port := range top100ports {
		wg.Add(1)
		go func(p int) {
			defer wg.Done()
			if IsportAlive(ip, p) {
